class BatchesController < ApplicationController
  before_filter :get_batch, only: %i[show filter_jobs chart_data cancel_jobs]
  ...
  def index
    ...
    @page    = params[:page] || 1
    @batches = @filter_query.scope.includes(:project,
                                            :test_cases,
                                            job_groups: [:test_results]).page(params[:page]).order('created_at desc')
  end
  ...
  def show
    if params[:view] == 'artifacts'
      ...
      @batch.jobs.each do |j|
        j.images.each do |i|
          name = i[0]
          file = i[1]
          @artifacts[name] = {} unless @artifacts[name]
    ...
    # Default view
      ...
      @jobs = @batch.jobs.order(:job_group_id, :job_name).group_by(&:job_group_id)
      render layout: 'scrollspy_menu'
    end
  end

  def cancel_jobs
    @batch.jobs.queued.each(&:cancel!)
  ...
  def create
    if batch_params['project_id'].empty?
      ...
      if @batch.save
        ...
        @batch.errors.messages.reject! { |k, _v| k == :job_groups }
        render action: 'new'
      end
    end
  end
  ...
  def filter_jobs
    @jobs = @batch.jobs.where(state: params[:state]).page(params[:page]).order(:created_at).group_by(&:job_group_id)
