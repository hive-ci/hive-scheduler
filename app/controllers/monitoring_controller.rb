class MonitoringController < ApplicationController
  def dashboard
    ...
    @queued_jobs = Job.includes(job_group: [:hive_queue], script: [:target])
                      .where(state: %i[queued reserved running])
                      .order(created_at: :asc)
                      .group_by { |j| j.job_group.hive_queue }

    @queues = @queues.sort { |a, b| (@queued_jobs[b] ? @queued_jobs[b].count : 0) <=> (@queued_jobs[a] ? @queued_jobs[a].count : 0) }

    @queues_with_no_workers = @queued_jobs.reject { |q, _j| @queues.include? q }.keys
  end
  ...
  def cancel_jobs
    ...
    jobs = Job.joins(:job_group)
              .where('job_groups.hive_queue_id = ? ', hive_queue_id)
              .where(state: %i[queued reserved running])
  ...
  def batch_counts
    months = params[:months].to_i.zero? ? 24 : params[:months].to_i
    @monthly_counts = Hive::UsageCounts.batch_counts(months: months)
  end

  def project_counts
    months = params[:months].to_i.zero? ? 18 : params[:months].to_i
    @project_counts = Hive::UsageCounts.project_counts(months: months)
  end

  def device_hours
    days = params[:days].to_i.zero? ? 6 : params[:days].to_i
  ...
  def job_status
    ...
    if jbs.count > 0
      ...
      by_project = jbs.group_by(&:project)
      ...
      @job_queue_data += @tmp_data.sort_by { |a| a[:queue] }
      ...
      @job_project_data += @tmp_data.sort_by { |a| a[:project] }
    else
      @job_queue_data = []
      @job_project_data = []
    end
  ...
  def parse_job_status(queue, data)
    not_cancelled = data.reject { |d| d.status == 'cancelled' }
    qd            = not_cancelled.select { |d| d.start_time.nil? }
    one_min       = not_cancelled.select { |d| d.start_time && d.start_time - d.created_at < 1.minute }
    two_min       = not_cancelled.select { |d| d.start_time && d.start_time - d.created_at < 2.minute }
    twenty_mins   = not_cancelled.select { |d| d.start_time && d.start_time - d.created_at < 20.minutes }

    results = {}
    %w[passed failed errored].each do |r|
