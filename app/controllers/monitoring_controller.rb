class MonitoringController < ApplicationController
  def dashboard
    ...
    @queued_jobs = Job.includes(job_group: [:hive_queue], script: [:target]).where(state: %i[queued reserved running]).order(created_at: :asc).group_by { |j| j.job_group.hive_queue }
  ...
  def cancel_jobs
    ...
    jobs = Job.joins(:job_group).where('job_groups.hive_queue_id = ? ', hive_queue_id).where(state: %i[queued reserved running])
