  ...
  module JobTestCases
    ...
    def associate_test_cases(*test_names)
      test_names.each do |t|
        associate_test_case_result(name: t, status: 'notrun')
    ...
    def associate_test_case_result(name: nil, urn: nil, status: 'unknown')
      project_id = job_group.batch.project_id
      ...
      raise "Couldn't find or create TestCase" unless tc

      # 2 Associate result with test case
      tr = TestResult.where(job_id: id, test_case_id: tc.id).last
      if tr && (tr.status == 'notrun' || tr.status == 'running')
        # i Find existing result and update
        tr.update(status: status)
      else
        # ii Create new result
        tr = TestResult.create(job_id: id, status: status, test_case_id: tc.id)
      end
      tr
    end

    def retriable_test_cases
      test_results.reject { |tr| tr.status == 'passed' }.collect(&:test_case)
    end
  end
end
