  ...
  class UsageCounts
    ...
    def self.project_counts(args = {})
      Hive::UsageCounts.count_per_month(args) do |start_time, end_time|
        Batch.where('created_at > ? and created_at <= ?', start_time, end_time)
             .pluck(:project_id).uniq.count
      end
    end
    ...
    def self.device_hours(args = {})
      ...
      (0..iterations).to_a.reverse.collect do |i|
        ...
        hours = Job.where('created_at > ? and created_at <= ?', start_time, end_time)
                   .collect { |j| j.end_time.to_i > 0 ? j.end_time.to_f - j.start_time.to_f : 0.0 }
                   .sum / 60 / 60
