namespace :hive do
  ...
  task gather_stats: :environment do
    ...
    FileUtils.mkdir_p stats_directory unless Dir.exist? stats_directory
    ...
    projects.each do |p|
      files["with_retries-#{p}"] = File.open("#{stats_directory}/#{date_label}_errors_with_retries-#{p.tr(' ', '_')}.csv", 'w')
    end
    ...
    31.times do |i|
      ...
      files[:with_retries].puts [
        date,
        parse_errors_with_retries(data: jbs,
                                  keys: keys[:with_retries],
                                  queue: nil)
      ].flatten.join(',')
      ...
      print "\r - [DONE]"
      print ' ' * (last_len - 6) if last_len > 6
      puts
    end
  ...
  def parse_errors_with_retries(options)
    ...
    latest = data.map do |j|
      j = j.replacement while j.replacement
      if j.retry_count < 3
    ...
    latest_not_cancelled = latest.reject { |d| d.status == 'cancelled' }
  ...
  def parse_job_start(data_in, queue)
    data = if queue
             data_in.select { |j| j.job_group.hive_queue.name == queue }
           else
             data_in
           end
